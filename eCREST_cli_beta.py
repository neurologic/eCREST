...
class ecrest:
    ...
    def neuroglancer_viewer(self,viewer_object=None):
        self.launched_viewer = True

        if viewer_object !=None:
            self.viewer = viewer_object
        ...
        elif viewer_object==None:
            # no object provided, launching and returning one

            viewer_object = neuroglancer.Viewer()

            self.viewer = viewer_object 

        # figure out a way for the following not to need to be re-done? so can keep prev. keybindings if changed
    ...
    def load_to_viewer(self):
        # load stuff specific to visualizing this cell
        ...
        if not missing_segs == set():
            ...
            with self.viewer.txn(overwrite=True) as s:
                ...
                for bs in missing_segs:
                    s.layers[layer].segment_colors[int(bs)] = '#d2b48c' # add as default color
